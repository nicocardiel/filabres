...
def delete_calibration(instrument, calibration):
    ...
    """
    print('> Image to be deleted {}'.format(calibration))

    # determine image type and night
    islash = calibration.find('/')
    if islash < 0:
        msg = 'ERROR: Invalid reduced filename: {}.\nCheck for missing path'.format(calibration)
        raise SystemError(msg)
    imagetype = calibration[:islash]
    iislash = calibration[(islash + 1):].find('/')
    night = calibration[(islash + 1):(islash + 1 + iislash)]
    ...
    for signature in signatures:
        ...
        for mjd in database[imagetype][signature].keys():
            ...
            if minidict['fname'] == calibration:
                particular_mjd = mjd
                particular_signature = signature
                break

    if particular_mjd is None:
        msg = 'ERROR: file {} not found in {}'.format(calibration, databasefile)
        raise SystemError(msg)
    ...
    if ncal_with_signature > 1:
        ...
        msg = 'ERROR: unexpected number of calibration images with required signature = {}'.format(ncal_with_signature)
        raise SystemError(msg)
    ...
    try:
        ...
        print('-> Deleting file {}'.format(calibration))
    except:
        print("Error while deleting file : ", calibration)
