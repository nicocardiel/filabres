...
def retrieve_calibration(instrument, redustep, signature, mjdobs,
    ...
    sortedkeys_, ssig = signature_string(expected_signature)
    ...
    if ssig in database[redustep]:
        ...
        print('* WARNING: signature {} not found for {} image'.format(
            ssig, redustep)
        )
        ierr = 1
        if redustep == 'bias':
